...
- The previous release's 22 HTTP checks passed against its built production Worker, with a healthy response afterward. The current 25-check suite passed against the local Worker/D1 runtime; it still needs a post-deploy public smoke check. A local Wrangler unread-request-body failure was reproduced and addressed by bounded body consumption before rejection; all origin, size and content-type checks remain enforced. Related upstream report: https://github.com/cloudflare/workers-sdk/issues/15203.
